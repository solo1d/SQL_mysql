...
mysql> SET  @LOCAL.变量 = '需要设置的值';     # 这样来临时修改变量,只影响这一个客户端的会话
...
#2. 创建一个存储过程,并在过程内创建一个局部变量 和打印这个局部变量.
    ...
    mysql>  CREATE PROCEDURE
            ...
            select  a;        # 输出和显示这个 局部变量a , 局部是可以这样来使用的
            end 
            $$;
    ...
    mysql> CREATE PROCEDURE
            ...
            SET b = 20;        #赋值为20
            SELECT b;          #这样是输出当前存储过程内容的b
#            SELECT @LOCAL.b;   # 这个是输出了 当前会话 本地变量 b 的值
            end
            $$;
