...
* 分析SQL的执行计划    :   
  * **mysql&gt;  explain + SQL 语句;**    可以模拟SQL优化器执行SQL语句, 从而让开发人员知道自己编写的SQL状态
    * 当使用多表联查的时候, 会优先进行 表内数据最少的来进行 **笛卡尔积** 计算.
    * 当 ID值不同, 那么ID值大的 优先执行, **\(本质: 在嵌套子查询时, 先查外层, 再查内层**\)
      * **ID值相同,   从上至下顺序执行**
      * **SLEECT\_TYPE   的分类**   
        *  **PRIMARY \(包含子查询的主查询语句,括号的最外层\),**    
        * **SUBQUERY \(包含子查询SQL中的 子查询, 非最外层\)** 
        * SIMPLE  \(简单查询, 不包含子查询,  和UNION\)
        * DERIVED  \(衍生查询,使用到了临时表, 比如 FROM子查询\)
        * union result :  告知开发人员, 那些表之间存在union 查询.
    * type: 索引类型, 类型
      * 性能排序 降序:  **system &gt; const &gt; eq\_ref &gt; ref &gt; range &gt; index  &gt; all**    \(前提是有索引\)
        * **system**  \(忽略这个\):    系统级别, 根本达不到.只有一条非常单纯的语句才可以达到.             结果只有一条数据
        * **const** : 仅仅能查到一条数据的SQL语句,  用于 Primary key 和 unique 索引.                  结果只有一条数据
        * **eq\_ref** :   唯一性索引 , 对于每个索引键的查询 , 返回匹配唯一行数据 \( 有且只有一个,不能多, 不能为0\);  常见于 主键和唯一索引      结果多条: 但是每条数据是唯一的
        * **ref:  非唯一性索引, 对于每个索引键的查询,返回匹配的所有行\(0,多\)                                   结果多条, 但是每条数据是0 或多条.**
        * **range** : 索引指定范围的行, WHERE 后面是一个范围查询\( berwen , in, &gt; &lt; = .  但是In有时候会失效,变成all\)
        * **index** :   查询全部索引中的数据, 跟着树查询,也就是遍历整个树
        * **all**  : 查询全部表中的数据, 将所有的列全部遍历一此 ,无索引状态, 搜索最慢
      * **possible\_keys**    可能用到的索引, 是一种预测, 不准确, 如果是NULL  则说明没有用到索引
      * **key**   实际使用到的索引
      * **key\_len**: 索引的长度, 作用: 用于判断 复合索引是否被完全使用了, \(utf8  一个char 占 3字节\), 如果索引字段可以为 NULL,   则会使用 一个字节来进行标示  \( char\(20\)  =  61 , varchar\(20\) = 63 &lt;有2个字节标示可变长度,一个标示NULL&gt;\)
    * **ref**  : 指明当前表 所参照的 字段
    * **rows**   被索引优化查询的数据个数  \(也就是通过索引查询到的数据个数\)
    * **extra** :  其他,   
      * **using  filesort** :  性能消耗大, 需要额外的一次排序 \(查询\),将所有数据查找出来,然后在排序一次 ,  也就是说 这个sql 语句效率太差了.       常见于 order by 语句中
        * 对于单索引,  如果排序和查找是同一字段, 则不会出现using filesort ,否则会出现.
        * 对于符合索引,  不能跨列. \(最佳左前缀\)
        * 避免 **using filesort** :  WHERE哪个字段,  就 ORDER BY 哪个字段.  按照符合索引的顺序使用,不要跨列使用.
      * **using temporary** :  需要额外再多用一张临时表, 有一次隐式查询, 一般出现在 group by 语句中.
        * 避免 using temporary  ;  WHERE 哪个字段, 就GROUP BY 哪个字段.
      * **using index** :   索引覆盖,  性能提升了.  原因:  不读取源文件,只从索引文件中获取数据 \(不需要回表查询\), 只要使用到的列全部都在索引中, 就是索引覆盖,
      * **using where**   需要回表查询
      * **impossible where :**   where子句永远为 false 
      * **using join buffer  :  mysql引擎使用了连接缓存, 这就说明语句太差了,动用了底层缓存.**
* **Mysql查询优化器会干扰用户的优化. 会修改优化后的代码, 并且出现概率事件.**

### 优化步骤和小结

* **单表优化,  两表优化, 三表优化**
* **按照解析过程进行优化, 和调整索引的顺序.**
* **一旦有的索引不再需要了,或者优化掉了,  那么一定要删除它.  防止干扰.**
* **最佳左前缀,  保持索引的定义和使用的顺序一致性**
* **索引需要逐步优化**
* **将含 In 的范围查询 , 放到WEHERE 条件的最后, 防止失效.**
* **索引添加到经常查询的字段上,  小表驱动的大表, 索引应该在 小表上   \(WHERE 小表=大表\)**
* **一般情况对于左外连接, 给左表加索引, 右外连接 给右表加索引**
* **exist 和 in**
  * SELECT .... FROM  表名  WHERE   exist     \(子查询\)
  * SELECT .... FROM  表名  WHERE  字段 in   \(子查询\)
    * 如果主查询的数据集大,  则使用In                    \(in 效率比 exist 高一些\) 
    * 如果子查询的数据集大,  则使用exist
* ORDER BY 优化 策略
  *  单路排序在使用时, 如果数据量大, 可以考虑调大buffer大小,  SET max\_length\_forsort\_data = 1024整数倍;     \(单位字节\)
  * 如果  max\_length\_forsort\_data 值太低, 则mysql 会自动从 单路排序切换到双路排序
  * 选择使用 单路或双路,  调整buffer 的容量大小, 
  * 避免使用  SELECT  \*  FROM . ......
  * 复合索引  不要跨列使用,  避免 using filesort
  * 保证全部的排序字段  排序的一致性\(都是升序 或  都是降序\)

### SQL语句优化, 就是在优化索引.  是一种概率层面的优化, 至于是否使用了我们的优化,需要通过 explain 来进行推测.
...
* 复合索引, 不要跨列或无序使用 \(最佳左前缀\) ,    
  * INDEX  \( a, b, c\) ---&gt;   WHERE a ..... and b ..... order by c ...
* 复合索引,  尽量使用全索引匹配
* 不要在索引上进行任何操作 \(计算,函数,类型转换\), 否则索引失效 
  * SELECT ...   WHERE  A.x= ....;   ---&gt;假设 A.x 是索引,    绝对不可以出现  WHERE  A.x \*3 .
* 复合索引 不能使用 不等于\( !=   &lt;&gt;  \)  或 is null \(is not null\) , 否则自身以及右侧索引所有全部失效
* 尽量使用索引覆盖, 可以避免概率问题, 就是遵循 复合索引的 索引顺序
* like 模糊查询 尽量以 "常量" 开头,  不要使用 '%' 开头, 否则索引失效.
  * 如果一定要使用 '%'  开头, 那么可以  SELECT 索引列  FROM 表  WHERE 索引列 like '%x%';
* 尽量不要使用类型转换\( 显示, 隐式\) ,  会使索引失效.
* 尽量不要在 WHERE语句中 使用 OR\(或\)  否则索引失效
